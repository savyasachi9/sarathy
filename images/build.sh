...
if [[ $BUILD_LIVE == 'no' ]]; then
    printf "Just building latest images & skipping building live\n"; exit 0;
fi
...
docker run --platform linux/${ARCH} -d --rm --privileged -it \
  -h sarathy --name ${LATEST_CONTAINER_NAME} ${LATEST_CONTAINER_IMAGE_TAG}
...
docker exec -it --user docker ${LATEST_CONTAINER_NAME} /bin/bash -c \
  "docker pull ${LANGTOOLS_CONTAINER_IMAGE_TAG_ALIAS} && sudo systemctl enable plt && sudo systemctl start plt" || true
...
docker run --platform linux/${ARCH} -d --rm --privileged -it \
  -h sarathy --name ${FINAL_CONTAINER_NAME} ${FINAL_CONTAINER_IMAGE_TAG}
...
docker ps; sleep 45;
docker exec -it --user docker ${FINAL_CONTAINER_NAME} /bin/bash -c \
  "kubectl get pods -A; docker ps; docker images;" || true
